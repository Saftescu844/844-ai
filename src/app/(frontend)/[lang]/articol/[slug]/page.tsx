...
export async function generateMetadata(props: {
  ...
  const imgUrl = img && typeof img === 'object' && img.url ? img.url : null

  const alt = articol.versiuneAlternativa
  const altObj = alt && typeof alt === 'object' ? alt : null
  ...
  return {
    ...
    openGraph: {
      ...
      ...(articol.publishedAt ? { publishedTime: articol.publishedAt } : {}),
    },
  }
}
    ...
    <article style={{ maxWidth: 720, margin: '0 auto', padding: '2.5rem 0' }}>
          ...
          {articol.galerie.map((g: any, i: number) => {
            ...
            return (
              <figure key={i} style={{ margin: 0 }}>
                ...
                {(g.caption || g.credit) && (
                  <figcaption
                    style={{ fontSize: 13, color: '#888', marginTop: 6, lineHeight: 1.4 }}
                  >
                    {g.caption}
                    {g.caption && g.credit ? ' · ' : ''}
                    {g.credit && <span style={{ fontStyle: 'italic' }}>{g.credit}</span>}
                  </figcaption>
                )}
              </figure>
            )
      ...
      {articol.videoUrl && videoEmbed(articol.videoUrl) && (
        <div style={{ marginTop: 32 }}>
          ...
          >
            <iframe
              src={videoEmbed(articol.videoUrl) as string}
              style={{
                position: 'absolute',
                top: 0,
                left: 0,
