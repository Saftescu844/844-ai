...
const scriptPath =
  resolve(
    process.cwd(),
    'scripts/flash-html-article-normalization-preview.ts',
  )

describe(
  'Flash HTML article normalization preview CLI',
  () => {
    it(
      'documents a read-only extraction-to-normalization path without downstream writes',
      async () => {
        const source =
          await readFile(
            scriptPath,
